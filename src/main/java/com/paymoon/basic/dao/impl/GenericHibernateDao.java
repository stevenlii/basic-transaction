...
//	}
	 ...
	 *            每页显示的记录数量
	 */
	public List<T> findPaged(final String sql, final int rowStartIdx,
			final int rowCount) {
		
		Assert.notNull(sql, "queryString不能为空");
	...
	public List<T> findByInParamter(final String sql,
	...
	public long findPagedConut(final String queryString) {
		Object count = findBySql(queryString).get(0);
		return (Long) count;
	}
